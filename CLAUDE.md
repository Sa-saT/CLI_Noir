...
├ noir-api/      … FastAPI バックエンド（全 29 Mission 実装済み・統合ワールド。553 tests green / ruff clean）
...
**実装タスクの復元**: セッション内タスクリストが空の場合、`context/04_task_backlog.md`（実装タスクの正。Part 1〜5 完了。冒頭の進捗サマリと「次回セッションの入り方」を見る）から未完了タスクを TaskCreate で復元してから着手する。タスク完了時は backlog 側のチェックボックスも [x] にする。
