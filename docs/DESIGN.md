...
| Mission クリア | 背景フェード → "Mission Complete!"（emerald）→ 次 Mission 導線。git push 成功の返却を受けてから発火。**演出中は独り言を保留**（ブリーフィングカードも閉じる）し、「次のミッションへ」で次 Mission ページへ移ってからクリア独り言 → 次 Mission の start 独り言を流す（サーバーは `mission_clear` → `story` の順で送る。2026-09-13） |
| ランクアップ | クリア演出を閉じた後（次 Mission ページ上）に辞令カード `RankUpEffect` を scene に重ねる（`rank_up` イベント。from/to のランクと新コマンド一覧）。クリックで受領し、その後に独り言が流れる（2026-09-13 実装）。右パネルの新コマンド点灯アニメーションは未実装 |
