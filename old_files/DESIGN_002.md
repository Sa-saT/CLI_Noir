...
- `TerminalView.vue` の内部構造: スクロールバックは `{ text, style }[]` を `v-for` で描画（`style` → CSS クラスの対応は § 2 のトークン表）。入力は native `<input>`（IME 対応のため。透明背景 + 緑文字でターミナル入力行に見せる）。keydown で `↑↓`=履歴 / `Tab`=補完 / `Ctrl+R`=逆検索 / `Ctrl+C`=中断 / `Ctrl+L`=クリア を処理。プロンプト文字列（§ 4）は WS の `state` から算出
