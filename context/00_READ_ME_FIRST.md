...
- **バックエンド `noir-api/`（FastAPI）**: 全 29 Mission 実装済み（542 tests green / ruff clean。2026-09-13 に git 編 23〜25 とサーバー編 26〜28 を追加、プレイ順序は id と独立）。**Part5 永続統合ワールド化は Phase A〜D 完了**（ユーザーごとに 1 つの `PlayerState`、区画の権限ゲート、`/root/case_file.sh` 動的合成、WS `/ws/terminal`・`GET /api/state/` は `PlayerState` を参照）。**Phase F（テスト移行・`MissionState` 廃止）も 2026-09-13 完了**。残るのは evaluator 内の旧形状分岐（`env_for`/`progress.flags`/`story`/`git_ops.is_world`/`_handle_resume` の else）の簡素化のみ
