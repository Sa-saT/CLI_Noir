...
## 常時ターミナル・独り言レイヤー・使用感の修正（2026-09-12〜13 確定）

Part5 統合ワールド化の Phase D（WS/API を `PlayerState` へ切替）を完了した上で、ユーザーの実プレイから次を確定した。

- **常時ターミナル**（FE3-02 の UX 判断）: ログイン後はターミナルが常に使える。WS 接続は `app.vue` がログインごとに 1 本張り、Mission ページはブリーフィング・ヒント・コマンド一覧の**表示切替専用**。「捜査を開始する」はブリーフィングカードを閉じるボタン。ルートの Mission ≠ 捜査中の Mission のときは注意文を出す
- **進行案内は探偵本人の独り言**（相棒キャラクターは不採用。設計指示書 § 11 機能 4 を上書き）: 一人称ノワール語り・話者ラベル無し。サーバー側 `MissionDef.story_beats`（`start`/`after`/`clear` トリガー）で発火し、scene 下辺の台詞窓にタイプライター表示（クリックで早送り、4 秒で自動送り）。停滞（60 秒無操作 or 連続エラー 3 回）は共通文言 + ヒントボタンの明滅
- **ヒントは独り言と別枠で「直接的なゴール説明」**: 1 ゴール / 2 必要コマンドと書式 / 3 コマンド列 + 各コマンドの簡易説明。ペナルティなし
- **独り言の文体**: 断定を避ける（「〜のはずだ」→「〜のはず」）。「!」「!?」は使ってよい（AUTHORING_GUIDE § 4「感嘆符は多用しない」を緩和）。Mission1〜3 の文言は `docs/Mission参照ファイル.md` で確定
...
- **クリア済み Mission は「再捜査」で遊び直せる**（2026-09-14 ユーザー方針「遊び返せる方が楽しみが続く」）: WS `focus` で舞台を初期状態に戻し、独り言・判定・push をその Mission に向ける（`progress.focused_mission_id` / `app/evaluator/replay.py`）。本編の進捗は動かさず `scores_replay` にベストを記録。隠し Mission（`secret_of`）も同じ仕組みで遊ぶ土台を用意（コンテンツは今後）
