...
残るのは → **ユーザーの実プレイでの文言・見た目・体験の確認**（Mission23〜29 の独り言、図鑑レイヤー、辞令→実習カード→独り言の順、
経過時計の位置、一覧カードの pt 表示）→ 画像素材（`moc/images/NEEDED_IMAGES.md`）到着時の `SCENE_IMAGES` 登録 →
`03_pending_items.md` の細目（`find` の絶対パス出力 / `&&` `;` / `git commit -m"msg"` / 右パネルの新コマンド点灯アニメ）。
E2E は `e2e02`（Mission3 捜査中）で。`detective01` / `e2e01` には触らない。
