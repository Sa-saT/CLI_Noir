...
**次回セッションの入り方（2026-09-13 3 回目の終業時）**: 本日は Phase F 完了 → 全 Mission の独り言/ヒント → Tab 補完・Ctrl+R・
探偵ランク → 追加エピソード（git 編 23〜25 / サーバー編 26〜28 / やらかし体験室 Mission29）→ ゲーム機能 2・5・10・11・12
（図鑑レイヤー・回想収集・現場実習カード・ご褒美）まで実装（542 tests green）。
次は → **ユーザーの実プレイでの文言・体験確認**（特に Mission23〜29 の独り言と judge の誤答文言、図鑑レイヤーと独り言の重なり）→
残るゲーム機能 3（スマート捜査ボーナス）/ 6（`man` ハンドブック）/ 7（タイムアタック演出）/ 8（リプレイ台帳）→ 画像素材
（`moc/images/NEEDED_IMAGES.md`）到着時の `SCENE_IMAGES` 登録。E2E は `e2e02`（Mission3 捜査中）で。`detective01` / `e2e01` には触らない。
ブラウザ検証の型は scratchpad の `e2e_*.py`（セッションごとに消えるので必要なら書き直す）。
