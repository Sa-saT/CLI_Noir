...
Mission1 のゴールデントランスクリプト方式を踏襲。Mission2・Mission3 とも統合
ワールド state（`tests.helpers.state_at_mission`）で検証する。Mission3 はローカル
FS 区画を持たず（_MISSION_AREAS に無い）、ssh amusement_park の到達性ゲート
（required_mission_id=3）を通した接続先 FS が舞台になる。
"""
...
def test_mission3_golden_transcript() -> None:
    ...
    out, s = _run(s, "git push")
    assert out == ["Mission Complete! Next mission unlocked."]
    assert 3 in s["mission_progress"]["completed"]
    assert progress.active_mission_id(s["mission_progress"]) == 4
