...
def test_mission14_golden_transcript() -> None:
    ...
    _, s = _run(s, 'echo "/root/mirror_hall/vault/real_deed.txt" > /root/mirror_hall/report.txt')
    ...
    assert 14 in s["mission_progress"]["completed"]
    assert progress.active_mission_id(s["mission_progress"]) == 15


def test_mission14_link_path_report_is_a_mirror() -> None:
    s = state_at_mission(14)
    _, s = _run(s, 'echo "/root/mirror_hall/deed_a.txt" > /root/mirror_hall/report.txt')
    out, s = _run(s, "sh /root/case_file.sh")
